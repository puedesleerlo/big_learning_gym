...
def lesson_text(lesson, sources, number):
    ...
    lines = [
        ...
        *lesson["objectives"],
        "# Intuition",
        *lesson["intuition"],
        "# " + lesson["worked_example"]["title"],
        lesson["worked_example"]["body"],
        "# Try it",
    ...
    return [
        {
            ...
            "html": paragraphs(
                [
                    ...
                    "Before this lesson: "
                    + ("; ".join(prerequisites) if prerequisites else "Nothing. Begin here."),
                ]
            )
        ...
        {
            "id": "example",
            "h": lesson["worked_example"]["title"],
            "html": paragraphs([lesson["worked_example"]["body"]]),
        },
        {
            "id": "try",
            "h": "Predict, try, explain",
            "html": paragraphs([lesson["experiment_task"], lesson["reflection"]]),
        },
        {
            "id": "remember",
            "h": "What to keep; what to question",
            "html": bullets(lesson["takeaways"]) + paragraphs(["Common mistake: " + lesson["pitfall"]]),
        },
        {
            "id": "paper",
            "h": "Your bridge to the research",
            "html": "<p>"
            + source_link(paper)
            + "</p>"
            + paragraphs(
                [
                    ...
                    bridge["why_it_matters"],
                    "Claim: " + bridge["claim"],
                    "Read first: " + bridge["read_first"],
                ]
            )
            + "<h4>Assumptions</h4>"
            + bullets(bridge["assumptions"])
            + "<h4>Limits</h4>"
            + bullets(bridge["limits"])
            + paragraphs([bridge["exercise"]]),
        },
        {
            "id": "practice",
            "h": "Check your understanding in the gym",
            "html": paragraphs(
                [
        ...
        {
            "id": "sources",
            "h": "Readings and provenance",
            "html": "<ul>"
            + "".join("<li>" + source_link(sources[s]) + "</li>" for s in lesson["source_ids"])
            + "</ul>"
            + paragraphs(
                [
...
def install(client, bundle, readings_dir=None):
    ...
    module_names = {
        entry["id"]: f"{module_ids[entry['id']]}: {entry['title']}" for entry in bundle["lessons"]
    }
    provenance = {
        "author": "Codex, at the learner's request",
        "method": "import",
        "rationale": "Original progressive causality curriculum; inspected primary sources; not learner performance. Bundle "
        + fingerprint,
        "reference_urls": [s["url"] for s in bundle["sources"]],
    }
    modules = [
        {
            "id": module_ids[entry["id"]],
            "title": entry["title"],
            "description": entry["question"],
            "prerequisites": [module_ids[p] for p in entry["prerequisites"]],
            "objectives": entry["objectives"],
            "estimated_minutes": entry["minutes"],
            "capabilities": [entry["id"]],
            "level": {"Foundations": "beginner", "Discovery": "intermediate", "Frontier": "frontier"}[
                entry["stage"]
            ],
        }
        ...
        GeneratedItem.model_validate(
            {
                ...
                "hint": lesson["takeaways"][0],
                "plain": lesson["question"],
                "source_fragment_ids": ["pending-upload"],
                "capabilities": [lesson["id"]],
    ...
    if existing.status_code == 404:
        ...
        if existing.json().get("provenance") != provenance:
            raise ValueError(
                "Course ID already exists with different provenance; inspect it before importing"
            )
    else:
    ...
    for number, lesson in enumerate(bundle["lessons"], 1):
        ...
        source = request(
            "POST",
            "/api/sources",
            data={"course_id": ident, "role": "instruction"},
        ...
        if "\n".join(f["text"] for f in fragments) != text:
            raise ValueError(
                "Source extraction changed authored text; review the reconstruction before import"
            )
        if source["reconstruction_status"] != "confirmed":
            request(
                "POST", f"/api/sources/{source['id']}/confirm", json={"expected_revision": source["revision"]}
            )
        ...
        guides.append(
            {
                "id": f"{ident}:{module}:guide",
                "expected_revision": 0,
                "course_id": ident,
                "code": module,
                "subtitle": lesson["title"],
                "blocks": guide_blocks(lesson, sources, module_names),
        ...
        for index, item in enumerate([checkpoint, *lesson["practice"]]):
            items.append(
                {
                    **item,
                    "id": f"{ident}:{module}:practice:{index}",
                    "expected_revision": 0,
                    "course_id": ident,
                    "module": module,
                    "pool": "practice",
    ...
    glossary_text = "# Causality glossary\n\nOriginal AI-assisted teaching definitions.\n\n" + "\n\n".join(
        "# " + term["term"] + "\n\n" + term["definition"] for term in bundle["glossary"]
    )
    glossary_source = request(
        "POST",
        "/api/sources",
        data={"course_id": ident, "role": "instruction"},
        files={"file": ("causality-glossary.md", glossary_text.encode(), "text/markdown")},
    )
    glossary_fragments = request("GET", f"/api/sources/{glossary_source['id']}/fragments")
    if "\n".join(fragment["text"] for fragment in glossary_fragments) != glossary_text:
        raise ValueError("Review the glossary extraction before importing")
    if glossary_source["reconstruction_status"] != "confirmed":
        request(
            "POST",
            f"/api/sources/{glossary_source['id']}/confirm",
            json={"expected_revision": glossary_source["revision"]},
        )
    if readings_dir:
        (readings_dir / "causality-glossary.md").write_text(glossary_text)
    for index, term in enumerate(bundle["glossary"]):
        ...
        match = next(
            (
                entry
                for entry in bundle["lessons"]
                if term["term"].lower() in lesson_text(entry, sources, 1).lower()
            ),
            bundle["lessons"][0],
        )
        terms.append(
            {
                "id": f"{ident}:term:{index}",
                "expected_revision": 0,
                "course_id": ident,
                "module": module_ids[match["id"]],
                "term": term["term"],
                "def": term["definition"],
                "distinguish": term.get("distinguish", ""),
                "why": term.get("why", ""),
                "source_fragment_ids": [
                    fragment["id"]
                    for fragment in glossary_fragments
                    if fragment["text"].startswith("# " + term["term"] + "\n")
                ],
                "provenance": provenance,
            }
        )
    for item in items:
        item["term_ids"] = [
            term["id"]
            for term in terms
            if term["module"] == item["module"] or term["term"].lower() in item["stem"].lower()
        ]
    ...
    guides.insert(
        0,
        {
            ...
            "course_id": ident,
            "code": "C01",
            "subtitle": "Start here: from everyday questions to frontier papers",
            "source_fragment_ids": lesson_fragments[first["id"]],
            "provenance": provenance,
            "blocks": [
                {
                    "id": "route",
                    "h": "A route you can follow from zero",
                    "html": paragraphs(
                        [
                    ...
                    + "<ol>"
                    + "".join(
                        f"<li>{html.escape(module_names[entry['id']])}</li>" for entry in bundle["lessons"]
                    )
                    + "</ol>",
                },
                {
                    "id": "courses",
                    "h": "Optional course materials",
                    "html": "<ul>"
                    + "".join(
                        "<li>" + source_link(s) + "</li>"
                        for s in bundle["sources"]
                        if "course" in s.get("kind", "").lower()
                    )
                    + "</ul>",
                },
            ],
    ...
    material_result = request(
        "POST",
        "/api/authoring/import",
        json={
            "idempotency_key": "causality:" + fingerprint,
            "course_id": ident,
            "guides": guides,
            "terms": terms,
            "items": items,
        ...
        "lessons": [
            {
                **{
                    ...
                    if k not in {"checkpoint", "practice", "source_ids", "prerequisites", "id"}
                },
                "id": module_ids[lesson["id"]],
                "prerequisites": [module_ids[p] for p in lesson["prerequisites"]],
                "source_ids": [lesson_sources[lesson["id"]]],
            }
            for lesson in bundle["lessons"]
        ],
    }
    from gym.lab_activity import LabWrite

    LabWrite.model_validate(lab_spec)
    current_lab = client.get(f"/api/labs/{ident}")
    if current_lab.status_code == 404:
        request("PUT", f"/api/labs/{ident}", json=lab_spec)
    elif not current_lab.is_success:
        raise ValueError(f"Cannot inspect the guided lab: HTTP {current_lab.status_code}")
    elif current_lab.json().get("provenance") != provenance:
        raise ValueError("Guided lab has changed; inspect it before importing")
    coursework = request("GET", "/api/coursework")
    rubric_title = "Causality: explain, test, and qualify a claim"
    rubric = next(
        (r for r in coursework["rubric"] if r.get("course_id") == ident and r["title"] == rubric_title), None
    )
    if not rubric:
        rubric = request(
            "POST",
            ...
            json={
                ...
                "capabilities": ["causal-research-reasoning"],
                "criteria": [
                    {
                        "name": "Question and causal model",
                        "weight": 0.25,
                        "anchors": [
                            "Vague association; unclear intervention or arrows",
                            "Defines intervention, outcome, comparison, and justified graph",
                        ],
                    },
                    {
                        "name": "Assumptions and reasoning",
                        "weight": 0.3,
                        "anchors": [
                            "Treats correlation as cause; hides identification assumptions",
                            "Explains what assumptions permit the conclusion and checks an alternative",
                        ],
                    },
                    {
                        "name": "Evidence and reproducibility",
                        "weight": 0.25,
                        "anchors": [
                            "Unsupported claim or unexplained result",
                            "Traceable calculation or reproducible experiment; cites the relevant paper",
                        ],
                    },
                    {
                        "name": "Limits and next test",
                        "weight": 0.2,
                        "anchors": [
                            "Claims more than the evidence supports",
                            "States uncertainty, a limitation, and a test that could change the conclusion",
                        ],
                    },
                ],
            },
    ...
    exercises = []
    for index, title in [
        (2, "Lab 1 · Explain a misleading comparison"),
        (4, "Lab 2 · What can these arrows tell us?"),
    ...
    exercises.append(
        {
            "title": bundle["capstone"]["title"],
            "kind": "project",
            "effort_minutes": 90,
            "prompt": bundle["capstone"]["prompt"],
            "source_ids": list(lesson_sources.values()),
        }
    )
    created_assignments = []
    for task in exercises:
        previous = next(
            (a for a in coursework["assignment"] if a["course_id"] == ident and a["title"] == task["title"]),
            None,
        )
    ...
    return {
        "course_id": ident,
        "guides": len(guides),
        "sources": len(lesson_sources) + 1,
        "terms": len(terms),
        "practice_items": len(items),
        "assignments": len(created_assignments),
        "rubric_id": rubric["id"],
        "already_imported": material_result.get("already_imported", False),
